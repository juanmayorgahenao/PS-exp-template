...
dscm_fieldbook <- readxl::read_excel(file.path(exp_path, 
                                        paste0("data/primary/raw/dscm/", exp_id, "_dscm_fieldbook.xlsx"))) |> 
  as_tibble() |> 
  janitor::clean_names()
...
dscm_meta <- dscm_fieldbook |>  
  ...
  mutate(extech_expedition_id = "DOEX0096_Palau",
...
dscm_meta <- readxl::read_xlsx(file.path(exp_path,
                                         "data/primary/raw/dscm/PLW_2024_dscm_fieldbook.xlsx")) |> 
  janitor::clean_names() |> 
...
dscm_meta <- dscm_meta |> 
  ...
  mutate(lead_operator = "Whitney Goodell",
         ...
         deployment_time = lubridate::ymd_hms(deployment_time, tz = "Pacific/Palau" ),
         recovery_time = lubridate::ymd_hms(recovery_time, tz = "Pacific/Palau"),
...
dscm_meta <- dscm_meta |> 
  select(ps_expedition_id, method, ps_site_id, location, sublocation,
         ...
         max_depth_m, bottom_temp_c, bottom_type,
         bait_type, bait_kg,
         ballast_type, ballast_kg,
         dscm_id, gtr_type, extech_expedition_id,
         lead_operator, 
         notes,
         highlights,
         everything()) |> 
