...
```{r recruits_clean, eval = TRUE}
recruits_fieldbook <-  readxl::read_xlsx(file.path(exp_path, 
  ...
  mutate(uvs_id = paste(exp_id, "uvs", formatC(uvs_numer, width = 2, flag = 0), 
                        sep = "_")) |> 
  group_by(uvs_id) |> 
  mutate(ps_site_id = paste(exp_id, "recruits",
                            formatC(cur_group_id(), 
                                    flag = 0, width = 2),
                            sep = "_")) |> 
  ungroup() |> 
  select(ps_site_id, uvs_id, transect, genus, size_cm, notes)

recruits_meta <- recruits_fieldbook |> 
  distinct(ps_site_id, uvs_id, transect) |> 
  left_join(lpi_meta |> 
              select(-ps_site_id,  -transect_length_m, -n_sections, -pts)) |> 
  mutate(method = "Coral recruitment",
         diver = "Molly Timmers",
...
```{r recruits_map, eval = TRUE, include = T}
tmp <- recruits_meta |> 
  filter(!is.na(lat)) |> 
...
```{r recruits_table, eval = TRUE, include = T}
recruits_meta |> 
  ...
  grand_summary_rows(columns  = contains(c("n_", "_m")),
                     ...
                     fmt = ~ fmt_number(., decimals = 0, use_seps = FALSE)) |> 
  tab_header(title = "Coral recruitment surveys per location, habitat, and exposure") |>
...
```{r recruits_export, eval = F}
bq_table_create(paste0("pristine-seas.metadata.",exp_id, "_recruits"), 
                fields = as_bq_fields(recruits_meta))

bigrquery::bq_table_upload(paste0("pristine-seas.metadata.",exp_id, "_recruits"), 
                           values = recruits_meta, fields = recruits_meta)
```
