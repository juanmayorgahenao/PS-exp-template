...
write_csv(expedition_meta, 
          file.path(exp_path, "reports/supplement/expedition_metadata.csv"))

expedition_meta |> 
  filter(!is.na(lat)) |> 
  mutate(local_time = as.character(local_time)) |> 
  sf::st_as_sf(coords = c("lon", "lat"), crs = 4326) |> 
  sf::st_write(file.path(exp_path, "data/primary/processed/expedition_metadata.gpkg"), append = F)
```

```{r exp_meta_tbl, eval = T, include = T}
#| label: tbl-meta
#| tbl-cap: Summary of workstations by location and method
...
#| label: fig-exp-meta
#| fig-cap: "Work sites by method used during the expedition."
#| cap-location: margin
tmp <- expedition_meta |> 
  filter(!is.na(lon)) |> 
  collect() |> 
  ...
  summarize(strata = if_else(is.na(depth_m), NA_character_, paste0(depth_m, "m",collapse = ", "))) |> 
