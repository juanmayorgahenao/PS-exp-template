...
```{r clams_clean, eval = TRUE}
clams_fieldbook <-  readxl::read_xlsx(file.path(exp_path, 
                                                   paste0("data/primary/raw/benthos/",
                                                          exp_id,
                                                          "_Inverts.xlsx")),
                                        sheet = "Giant Clam Sizes") |> 
  as_tibble() |> 
  janitor::clean_names() |> 
  ...
  mutate(uvs_id = paste(exp_id, "uvs", formatC(uvs_numer, width = 2, flag = 0), 
                        sep = "_")) |> 
  group_by(uvs_id) |> 
  mutate(ps_site_id = paste(exp_id,"clams",
                            formatC(cur_group_id(), 
                                    flag = 0, width = 2),
                            sep = "_")) |> 
  ungroup() |> 
  select(ps_site_id, uvs_id, transect = strata, depth, species, size,)

clams_meta <- clams_fieldbook |> 
  distinct(ps_site_id, uvs_id, transect) |> 
  left_join(lpi_meta |> 
              select(-ps_site_id,  -transect_length_m, -n_sections, -pts)) |> 
  mutate(method = "Giant clam demographics",
         diver = "Molly Timmers",
...
```{r clams_map, eval = TRUE, include = T}
tmp <- clams_meta |> 
  filter(!is.na(lat)) |> 
...
```{r clams_table, eval = TRUE, include = T}
clams_meta |> 
  ...
  grand_summary_rows(columns  = contains(c("n_", "_m")),
                     ...
                     fmt = ~ fmt_number(., decimals = 0, use_seps = FALSE)) |> 
  tab_header(title = "Giant Clam surveys per location, habitat, and exposure") |>
...
```{r clams_export, eval = F}
bq_table_create(paste0("pristine-seas.metadata.",exp_id, "_clams"), 
                fields = as_bq_fields(clams_meta))

bigrquery::bq_table_upload(paste0("pristine-seas.metadata.",exp_id, "_clams"), 
                           values = clams_meta, fields = clams_meta)
```
