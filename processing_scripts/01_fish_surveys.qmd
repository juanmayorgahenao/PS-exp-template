...
genus_to_common_fam <- fish_trophic_group |> 
  distinct(genus, genus_common_family = common_family)

fish_lut <- fish_lut |> 
  left_join(genus_to_common_fam) |> 
  mutate(common_family = coalesce(common_family, genus_common_family)) |> 
  select(-genus_common_family)
...
hc_data <- summary_by_taxa |> 
  mutate(n = 1) |> 
  mutate_if(is.numeric, round,2) |> 
  data_to_hierarchical(c(order, common_family), n)
...
ggstatsplot::grouped_ggbetweenstats(data = summary_by_transect, x = exposure, y = n_taxa, grouping.var = location,
                                    type = "nonparametric",
                                    xlab = "", ylab = "",
...
hc_data <- summary_by_taxa |> 
  ...
  data_to_hierarchical(c(common_family, taxon_valid_name), gr_m2)
...
tmp_plot <- biomass_by_transect_and_taxa |> 
  ...
  ggthemes::theme_hc()+
  paletteer::scale_fill_paletteer_d("ggthemes::Tableau_10", na.value = "grey")+
...
tmp_plot <- summary_by_taxa |> 
  ...
  ggiraph::geom_point_interactive(aes(x = freq, 
                                      ...
                                      col = trophic, 
                                      tooltip = tooltip))+
  paletteer::scale_colour_paletteer_d("ggthemes::Tableau_10", na.value = "grey")+
  scale_y_log10()+
...
```{r pcoa_plot, include = T, eval = T, fig.width = 10, fig.height = 10}
#| label: fig-pcoa
...
ggplot() +
  ...
  geom_segment(data = gr_m2_pcoa_c_spp_scores %>% 
      ...
      paletteer::scale_colour_paletteer_d(palette = "MetBrewer::Juarez")+
      scale_alpha(range=c(0.6,1))

ggsave(file.path(exp_path, "figures", "fish_biomass_pcoa_by_location.pdf"), width = 10, height = 10)
```
