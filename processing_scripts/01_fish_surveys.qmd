...
uvs_meta <- uvs_meta |> 
  ...
  select(ps_expedition_id,  uvs_id, location, date, local_time, lat, lon, habitat, exposure, alternative_site_id, everything(), -dd_mm_yyyy, -sand_number, -sand_m, -ps_station_id, -team_lead)
```
...
fish_meta <- fish_meta |> 
  mutate(method = "Fish survey",
         ...
         depth_strata = case_when(depth_m <= 5 ~ "Supershallow",
                                  ...
                                  depth_m > 15 ~ "Deep")) |> 
  left_join(uvs_meta) |> 
  select(ps_expedition_id, method, ps_site_id, location, date, local_time, lat, lon, habitat, exposure, transect_id, depth_strata, depth_m, transect_length_m, n_reels, uvs_id , alternative_site_id, diver) |> 
  ungroup() |> 
  arrange(ps_site_id)
```
...
```{r fish_export}
bq_table_create("pristine-seas.fish_surveys.transects", 
                fields = as_bq_fields(fish_meta))

bigrquery::bq_table_upload("pristine-seas.fish_surveys.transects", 
...
```{r}
SIO_fish_data <- read_csv(file.path(ps_science_path, 
                                         "expeditions", 
                                         "SLI-2023", 
  ...
  distinct(taxon, taxon_code = new_name, common_family, consumer, trophic, trophic_detailed, family, genus, 
           lmax_cm, a_cm, b_cm, ltl_rat)

fish_lut <- fish_lut |> 
  left_join(SIO_fish_data |> 
              select(taxon_code, a_cm, b_cm, ltl_rat)) 
  
na_params <- fish_lut |> 
  filter(is.na(a_cm)) |> 
  select(-c(a_cm, b_cm, ltl_rat)) |> 
  left_join(SIO_fish_data |> 
              select(taxon_valid_name = taxon, a_cm, b_cm, ltl_rat)) 
...
fish_lut <- fish_lut %>% 
  group_by(genus) %>% 
  mutate_at(vars(a_cm, b_cm, ltl_rat), 
            .funs =  impute.mean) %>% 
  mutate_all(~ifelse(is.nan(.), NA, .)) %>% 
  ungroup()
...
fish_lut$a_cm[is.na(fish_lut$a_cm) & str_detect(fish_lut$taxon, "Apogon")] <- 0.01545000
fish_lut$b_cm[is.na(fish_lut$b_cm) & str_detect(fish_lut$taxon, "Apogon")] <- 3.12126
fish_lut$ltl_rat[is.na(fish_lut$ltl_rat) & str_detect(fish_lut$taxon, "Apogon")] <- 0.95

fish_lut <- fish_lut |> 
  mutate(a_cm = coalesce(a_cm, a),
         b_cm = coalesce(b_cm, b),
         ltl_rat = coalesce(ltl_rat, 1)) 
```
...
fish_lut |> 
  ...
  rows_complete(columns = c(taxon_code, taxon_valid_name, max_length_tl, a, b, a_cm, b_cm, ltl_rat, common_family, consumer, trophic, trophic_detailed)) |> 
...
```{r obs_clean}
fish_lut <- read_csv(file.path(exp_path, "data/primary/processed/fish_taxa_lut.csv"))
...
fish_obs <- fish_obs |> 
  left_join(fish_lut |> 
              select(taxon_code, taxon_valid_name, max_length_tl, a_cm, b_cm, ltl_rat, common_family, trophic))
```
...
biomass_by_obs <- fish_obs |> 
  ...
  mutate(ps_expedition_id = "PLW_2024",
         biomass_gr = n_ind*a_cm*(avg_tl_cm*ltl_rat)^b_cm,
         ind_m2 = if_else(avg_tl_cm <= 20, n_ind/50, n_ind/100),
         gr_m2 = if_else(avg_tl_cm <= 20, biomass_gr/50, biomass_gr/100)) |> 
  select(ps_expedition_id, ps_site_id, depth_strata, depth_m, diver, reel, taxon_valid_name, avg_tl_cm, n_ind, ind_m2, biomass_gr, gr_m2) 

write_csv(biomass_by_obs, 
         file.path(exp_path, "data/primary/processed/fish_observations.csv"))

# bq_table_create("pristine-seas.fish_surveys.observations", 
#                 fields = as_bq_fields(biomass_by_obs))
# 
# bigrquery::bq_table_upload("pristine-seas.fish_surveys.observations",
...
```{r bq_import, eval = T}
transects <- tbl(bq_connection, "pristine-seas.fish_surveys.transects") |> 
  filter(ps_expedition_id == exp_id) |> 
  collect()
...
observations <- read_csv(file.path(exp_path, "data/primary/processed/fish_observations.csv"))
```

```{r summarize, eval = T}
biomass_by_reel_and_taxa <- observations |> 
  group_by(ps_site_id, depth_strata, depth_m, diver, reel, taxon_valid_name) |> 
  summarise(across(c("ind_m2",  "gr_m2"),
                   .fns =  ~sum(.x, na.rm = T))) |> 
  ungroup() |> 
...
summary_by_reel <- biomass_by_reel_and_taxa |> 
  group_by(ps_site_id, depth_strata, diver, reel) |> 
  summarize(n_taxa = n_distinct(taxon_valid_name[ind_m2 > 0]),
            ind_m2 = sum(ind_m2),
            gr_m2 = sum(gr_m2)) |> 
  left_join(transects |> distinct(ps_site_id, location, exposure, habitat)) |> 
  ungroup()
...
summary_by_transect <- biomass_by_transect_and_taxa |> 
  ...
  summarize(n_taxa = n_distinct(taxon_valid_name[ind_m2 > 0]),
            across( c("ind_m2",  "gr_m2"), .fns = sum, na.rm = T)) |>
  ungroup() |> 
  left_join(transects |> 
              distinct(ps_site_id, location, exposure, habitat)) |> 
...
```{r exp_tbl, eval = T, include = T}
#| label: tbl-exp-summary
#| tbl-cap: Summary of results
#| tbl-cap-location: top
exp_summary <- summary_by_transect |> 
  ...
  summarize(sites = n_distinct(ps_site_id[ind_m2 > 0]),
            ...
            n_reels = sum(n_reels),
            n_taxa = mean(n_taxa),
            ind_m2_avg = mean(ind_m2),
            ind_m2_sd = sd(ind_m2),
            gr_m2_avg = mean(gr_m2),
            gr_m2_sd = sd(gr_m2)) |> 
  mutate_if(is.numeric, round, 2) 

exp_summary_by_strata <- summary_by_transect |> 
  mutate(group = "By strata",
         var = depth_strata) |> 
  group_by(group, var) |> 
  summarize(sites = n_distinct(ps_site_id),
            n_transects = n(),
            n_reels = sum(n_reels),
            n_taxa = mean(n_taxa),
            ind_m2_avg = mean(ind_m2),
...
exp_summary_by_exposure <- summary_by_transect |> 
  mutate(group = "By exposure",
         var = exposure) |> 
  group_by(group, var) |> 
  summarize(sites = n_distinct(ps_site_id),
            n_transects = n(),
...
bind_rows(exp_summary_by_exposure, exp_summary_by_strata, exp_summary) |> 
  gt::gt(rowname_col = "var", groupname_col = "group") |> 
  cols_merge(columns = starts_with("ind_m2"),
             pattern = "{1} <<({2})>>") |> 
  cols_merge(columns = starts_with("gr_m2"),
             pattern = "{1} <<({2})>>") |> 
  tab_header(title = "Average species richness, abundance, and biomass") |>
   tab_footnote(
    footnote = "Standard deviation in parenthesis",
    cells_column_labels(columns = c(ind_m2_avg, gr_m2_avg))) |> 
  opt_align_table_header(align = "left") |> 
  tab_style(style = cell_text(weight = "bold"),
            locations = cells_row_groups()) |> 
  tab_options(ihtml.use_compact_mode = TRUE,
  ...
  cols_label_with(fn = ~str_replace(.x, "n_", "# ") |> 
                    str_to_title()) |> 
  cols_label("ind_m2_avg" ~ "Abundance (ind/m2)",
             "gr_m2_avg" ~ "Biomass (g/m2)") |> 
  opt_align_table_header(align = "left") 
```
...
#| tbl-cap-location: top
(tbl_summary1 <- summary_by_transect |> 
...
gt::gtsave(tbl_summary1, 
           filename = file.path(exp_path, "tables", "fish-surveys/location_summary.docx"))
```
...
```{r static_treemap, include = T, eval = T}
fish_taxatreemap <- summary_by_taxa |> 
  group_by(order, family) |> 
  summarize(n_taxa = n_distinct(taxon_valid_name[freq > 0]),
  ...
  scale_fill_manual(values = sample(paletteer_d("ggsci::default_igv", direction = -1), 51))

ggsave(fish_taxatreemap,
       filename = file.path(exp_path, "figures/fish-surveys/fish_taxatreemap.pdf"), dpi = 300,
       width = 10)
```
...
ggstatsplot::ggbetweenstats(data = summary_by_transect, 
                            x = location, 
                            y = n_taxa, 
                            type = "nonparametric",
                            title = "Species richness by island", 
...
ggsave(file.path(exp_path,
                 "figures/fish-surveys/diversity_by_island.pdf"), 
       dpi = 300, width = 10)
```
...
(tbl_fish_diversity_adonis <- broom::tidy(fish_diversity_adonis) %>% 
...
gt::gtsave(tbl_fish_diversity_adonis,
           filename = file.path(exp_path, "tables", "fish-surveys/diversity_adonis.docx"))
```
...
ggsave(file.path(exp_path,
                 "figures/fish-surveys/diversity_by_island_and_exposure.pdf"), 
       dpi = 300, width = 12)
```
...
ggstatsplot::ggbetweenstats(data = summary_by_transect, x = location, y = ind_m2, 
                            ...
                            xlab = "", ylab = "Individuals per m2",
                            ggplot.component = list(scale_color_manual(values = ps_palettes$location)))

ggsave(file.path(exp_path,
                 "figures/fish-surveys/abundance_by_island.pdf"), 
       dpi = 300, width = 10)
```
...
(tbl_fish_abundance_adonis <- broom::tidy(fish_abundance_adonis) %>% 
  mutate_if(is.numeric, round, 3) %>% 
  ungroup() |> 
  gt(rowname_col = "term")  |> 
...
gt::gtsave(tbl_fish_abundance_adonis,
           filename = file.path(exp_path, "tables", "fish-surveys/abundance_adonis.docx"))
```
...
ggsave(file.path(exp_path,
                 "figures/fish-surveys/abundance_by_island_and_exposure.pdf"), 
       dpi = 300, width = 12, height = 13)
```
...
ggstatsplot::ggbetweenstats(data = summary_by_reel, x = location, y = gr_m2, 
                            type = "nonparametric",
...
ggsave(file.path(exp_path,
                 "figures/fish-surveys/biomass_by_island.pdf"), 
       dpi = 300, width = 12)
```
...
#| tbl-cap-location: top
fish_biomass_adonis <- vegan::adonis2(summary_by_transect %>% 
...
(tbl_fish_biomass_adonis <- broom::tidy(fish_biomass_adonis) %>% 
  mutate_if(is.numeric, round, 3) %>% 
  ungroup() |> 
  gt(rowname_col = "term")  |> 
...
gt::gtsave(tbl_fish_biomass_adonis,
           filename = file.path(exp_path, "tables", "fish-surveys/biomass_adonis.docx"))
...
ggsave(file.path(exp_path,
                 "figures/fish-surveys/biomass_by_island_and_strata.pdf"), 
       dpi = 300, width = 12)
```
...
ggsave(file.path(exp_path,
                 "figures/fish-surveys/biomass_by_island_and_exposure.pdf"), 
       dpi = 300, width = 12, height = 13)
```
...
#| tbl-cap-location: top
(tbl_biomass_by_exposure <- summary_by_transect |> 
...
gt::gtsave(tbl_biomass_by_exposure,
           filename = file.path(exp_path, "tables", "fish-surveys/summary_by_location_and_exposure.docx"))
```
...
taxa_colors <- summary_by_taxa |> 
  ...
  mutate(color_fam = highcharter::colorize(common_family, 
  ...
  mutate(n_taxa = n_distinct(taxon_valid_name),
         color_sp = highcharter::colorize(taxon_valid_name,
                             colors = create_family_pal(col = color_fam, n = n_taxa))) |> 
  ungroup()

hc_data <- summary_by_taxa |> 
  left_join(taxa_colors |> 
              select(taxon_valid_name, color_fam, color_sp)) |> 
...
```{r static_treemap_biomass, include = T, eval = T}
biomass_treemap <- hc_data |> 
  group_by(order, family) |> 
  ...
  ggplot(aes(area = gr_m2, 
             label = paste(family, gr_m2, sep = "\n"),
             fill = order,
             subgroup = order)) +
  treemapify::geom_treemap(show.legend = T)+
  treemapify::geom_treemap_text(colour = "black", place = "middle", reflow = T, min.size = 3)+
  labs(fill = "",
       title = "Fish biomass by family. Color represents taxonomic order")+
  scale_fill_manual(values = sample(paletteer_d("ggsci::default_igv", 
                                                direction = -1), 18))+
  theme(legend.position = "bottom", legend.text = element_text(size = 7))

ggsave(biomass_treemap,
       filename = file.path(exp_path, "figures/fish-surveys/biomass_treemap.pdf"),
       dpi = 300,
       width = 10)
```
...
#| tbl-cap-location: top
(tbl_summary_by_taxa <- summary_by_taxa |> 
  ...
  ungroup() |> 
  mutate(total_abundance_pct = 100*total_abundance/sum(total_abundance),
         total_biomass_pct = 100*total_biomass/sum(total_biomass)) |> 
  mutate_if(is.numeric, round, 2) |> 
  arrange(desc(total_biomass)) |> 
  gt(groupname_col = "class", rowname_col = "order") |> 
  cols_merge(columns = starts_with("total_abundance"),
             pattern = "{1} <<({2} %)>>") |> 
  cols_merge(columns = starts_with("total_biomass"),
             pattern = "{1} <<({2} %)>>") |> 
  ...
  opt_align_table_header(align = "left") ) |> 
   tab_footnote(
    footnote = "Percent contribution in parenthesis",
    cells_column_labels(columns = c(total_abundance, total_biomass))) 

gt::gtsave(tbl_summary_by_taxa, 
           filename = file.path(exp_path, "tables", "fish-surveys/summary_by_family.docx"))
```
...
ggsave(plot = tmp_plot, 
       filename = file.path(exp_path, "figures/fish-surveys/biomass_by_island_and_family.pdf"),
       height = 10)
```
...
```{r}
biomass_by_transect_and_trophic <- biomass_by_transect_and_taxa |> 
  left_join(fish_lut |>
              distinct(taxon_valid_name, trophic)) |>
  group_by(ps_site_id, depth_strata, diver, trophic) |> 
  summarize(ind_m2  = sum(ind_m2), 
            gr_m2 = sum(gr_m2)) |> 
  ungroup() 
  
biomass_by_location_and_trophic <- biomass_by_transect_and_trophic |> 
  left_join(transects |> distinct(ps_site_id, location)) |> 
  group_by(location, trophic) |> 
  summarise(ind_m2  = mean(ind_m2),
            gr_m2 = mean(gr_m2)) |> 
  ungroup()  
  
biomass_by_trophic <- biomass_by_transect_and_trophic |> 
  left_join(transects |> distinct(ps_site_id, location)) |> 
  group_by(trophic) |> 
  summarise(ind_m2  = mean(ind_m2),
            gr_m2 = mean(gr_m2)) |> 
...
biomass_by_location_and_trophic |> 
  select(-ind_m2) |> 
  pivot_wider(names_from = location, values_from = gr_m2) |> 
  left_join(biomass_by_trophic |> 
              select(-ind_m2)) |> 
  rename(Overall = gr_m2) |> 
  mutate(exp = "SLI") |> 
  mutate_if(is.numeric, round, 2) |> 
  gt::gt(rowname_col = "trophic", groupname_col = "exp") |>  
  gt::summary_rows(columns = contains(c("H", "F", "M", "S", "T", "P", "O")),
               fns = list(list(label = md("*Total*"), fn = "sum")),
               fmt = ~ fmt_number(., decimals = 0, use_seps = FALSE)) 
```
...
ggsave(plot = tmp_plot, 
       filename = file.path(exp_path, "figures/fish-surveys/biomass_by_trophic_and_strata.pdf"), 
       height = 6)
```
...
```{r top_30_species,  include = T, eval = T}
#| label: tbl-top-sp
#| tbl-cap: Top 30 fish species
#| tbl-cap-location: top
summary_by_taxa |> 
  mutate(ind_m2_pct = 100*ind_m2/sum(ind_m2),
         gr_m2_pct = 100*gr_m2/sum(gr_m2)) |> 
  mutate_if(is.numeric, round, 2) |> 
  arrange(desc(gr_m2)) |> 
  slice_head(n = 30) |> 
  select(-class, -order, -family, -genus) |>
  gt(groupname_col = "trophic", rowname_col = "common_family") |> 
  cols_merge(columns = starts_with("ind_m2"),
             pattern = "{1} <<({2} %)>>") |> 
  cols_merge(columns = starts_with("gr_m2"),
             pattern = "{1} <<({2} %)>>") |> 
  tab_stubhead(label = "Trophic/Family") |> 
  gt::tab_header(title = "Top 30 fish species with highest contribution to overall fish biomass") |> 
  opt_align_table_header(align = "left") |> 
  tab_style(style = cell_text(align = "left", indent = px(20)),
            locations = cells_stub()) |> 
  tab_style(style = cell_text(weight = "bold"),
  ...
  tab_options(ihtml.use_compact_mode = TRUE,
              data_row.padding = px(5)) |> 
  cols_label("taxon_valid_name" ~ "Taxon",
             "min_tl_cm" ~ "Min length (cm)",
             "mean_tl_cm" ~ "Mean length (cm)",
             "max_tl_cm" ~ "Max length (cm)",
             "freq" ~ "Frequency (% stations)",
             "ind_m2" ~ "Abundance (ind/m2)",
             "gr_m2" ~ "Biomass (g/m2)") |> 
  opt_align_table_header(align = "left") |> 
  tab_footnote(footnote = "Percent contribution in parenthesis",
               cells_column_labels(columns = c(gr_m2, ind_m2))) 
```
...
ggsave(file.path(exp_path, "figures", "fish-surveys/biomass_pcoa_by_location.pdf"), 
       width = 10, height = 10)
```
